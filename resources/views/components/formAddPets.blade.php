<div class="{{ $errors->any() == false ? 'hidden' : 'block' }}" id="formAddPets">
    <form action="{{ route('addPet') }}" method="post" class="flex flex-col gap-y-3 w-1/3 m-auto">
        ...
        <div class="flex flex-row">
            ...
            <input class="radio" type="radio" name="maleM" id="maleM">
            <label class="px-3" for="maleW" id="maleW_label">{{ __('Женский') }}</label>
            <input class="radio" type="radio" name="maleW" id="maleW">
        </div>
        ...
        @enderror
        <input class="flex m-auto btn bg-[#3d4451]" type="submit" value="Добавить">
    </form>
</div>